...
module.exports = function (User) {
  ...
  let disabledPrototypesRemoteMethods = ['patchAttributes', 'verify']
  let enabledRemoteMethods = ["create", "find", "findOrCreate", "login", "logout", "resetPassword","replaceById",'findById']
  ...
  var count = 0;
  User.addToCart2 = function (productInfo, cb) {
  ...
  User.addToCart = function (productInfo, cb) {
    ...
    console.log('id & quantity after addtoCart ',id,quan);
  ...
  User.placeOrder = function (info, cb) {
    ...
    //order tracking..
    let trackOrder = [];
    trackOrder.push('A req to place order has been sent to e-commerce.');
    ...
    User.findById(userId, function (err, user) {
      ...
      if (isTxPossible) {
        ...
        request.post(url2, {
          ...
          if(body == true){
          status = 'Deduction of '+t_cost+' taka from '+user.ac_holder+' account is success.';
           }
          console.log(status);
            trackOrder.push(status);
        });
                ...
                function (err, response, addedCart) {
                  if (!err && response.statusCode == 200) {
                    ...
                    request.post(url4, {
                      ...
                      if(body == true){
                      status = 'A Request to supply cart product with tx_record is success.';
                     }
                      console.log(status);
                      trackOrder.push(status);
                      trackOrder.push('Products are delivered.');
                      cb(null, trackOrder);
                    })
